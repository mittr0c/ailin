...
from .game import Game, Player,PlayerQ ,AiPlayer
...
shortcut("国际象棋对战", ["--battle"], aliases={"国际象棋双人","国际象棋","chess"}, rule=is_group)
...
def new_player(event: MessageEvent) -> Player:
    ...
    return Player(event.user_id, f'{查chess段位(str(event.user_id))}{str(event.user_id)}')

def new_playerQ(event: MessageEvent) -> PlayerQ:
    #return Player(event.user_id, event.sender.card or event.sender.nickname or "")
    return Player(event.user_id, str(event.user_id))
...
async def handle_chess(matcher: Matcher, event: MessageEvent, argv: List[str]):
    ...
    if not games.get(cid, None):
        ...
        playerQ = new_playerQ(event)
        if options.black:
            game.player_black = player
            game.playerQ_black = playerQ
        else:
            game.player_white = player
            game.playerQ_white = playerQ
    ...
    if not game.player_last:
        ...
        加chess排位分(str(event.user_id),0.05)
        ...
        if result == Termination.CHECKMATE:
            ...
            if game.is_battle:
                ...
                if winner == chess.WHITE:
                    ...
                    加chess排位分(game.playerQ_white,3)
                    加chess排位分(game.playerQ_black,1)
                else:
                    msg += f"，恭喜 {game.player_black} 获胜！"
                    加chess排位分(game.playerQ_white,1)
                    加chess排位分(game.playerQ_black,3)
            else:
