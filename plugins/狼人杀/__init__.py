...
cg = on_command("创建狼人杀", aliases={"创建", "开房"})    # 初始化
zd = on_command("自定义", aliases={"自定"})     # 自定义配置, 后面带参数
jn = on_command("参加狼人杀", aliases={"参加", "加入狼人杀"})  # 参加
ks = on_command("开始狼人杀", aliases={"狼人杀"})  # 进入游戏线程
...
async def _(bot: Bot, event: MessageEvent):
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            cf.group = group
            cf.bot = bot
            await cg.send("游戏房间初始化成功,发“参加狼人杀”即可参加")
    else:
...
async def _(event: MessageEvent, arg: Message = CommandArg()):
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            await zd.send("游戏房间不存在,请先发“创建狼人杀”")
    else:
    ...
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            if qq in finalpool.member_group or qq in cf.joiner:
                await jn.finish("你已参加游戏.不可重复参加，发“开始狼人杀”即可开始")
            if len(cf.joiner) == 12:
...
async def _(event: MessageEvent):
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            await ks.finish("游戏房间不存在,请先发“创建狼人杀”")
    else:
...
async def _(event: MessageEvent):
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            await rs.finish("游戏房间不存在,请先发“创建狼人杀”")
    else:
...
async def _(event: MessageEvent):
    if event.message_type == 'group':
        ...
        if group in finalpool.configs:
            ...
            await gg.finish("游戏房间不存在,请先发“创建狼人杀”")
    else:
...
skill = on_command("skill", aliases={"守", "摄梦", "诅咒", "刀", "查", "毒"})  # skill回合
